...
<div class="form-group {{ $errors->has('city') ? 'has-error' : ''}}">
    ...
    <input class="form-control" name="city" type="text" id="city" value="{{ isset($masterclasses->city) ? $masterclasses->city : old('city')}}" >
    {!! $errors->first('city', '<p class="help-block">:message</p>') !!}
</div>
<div class="form-group {{ $errors->has('date_begin') ? 'has-error' : ''}}">
    <label for="date_begin" class="control-label">{{ 'Дата початку' }}</label>
    <input class="form-control" name="date_begin" type="date" id="date_begin" value="{{ isset($masterclasses->date_begin) ? $masterclasses->date_begin : old('date_begin')}}" >
...
<div class="form-group {{ $errors->has('date_end') ? 'has-error' : ''}}">
    ...
    <input class="form-control" name="date_end" type="date" id="date_end" value="{{ isset($masterclasses->date_end) ? $masterclasses->date_end : old('date_end')}}" >
...
<div class="form-group {{ $errors->has('date') ? 'has-error' : ''}}">
    ...
    <input class="form-control" name="date" type="date" id="date" value="{{ isset($masterclasses->date) ? $masterclasses->date : old('date')}}" >
    ...
    <script>
        ...
        function initMap() {
            ...
            let map = new google.maps.Map(document.getElementById('map'), {
                center: {lat: myLat ? myLat : 49.234, lng: myLng ? myLng : 28.466},
                zoom: 8
            });
